...
select * from dept_locations
where dlocation not in ('Houston','Stafford');

select * from dept_locations where dlocation !='Houston' and dlocation != 'Stafford';
...
select * from works_on order by pno desc,hours asc;
...
select dno,count(*) as Noof_male_employee from employee where sex='M' group by dno;

select dno,dname ,count(*) as Nomof_male_employees from employee inner join department on dno=dnumber where sex='M' group by dno order by dno;
...
select essn ,count(pno) from works_on group by essn order by count(pno) desc;

select essn, concat(fname,'  ',minitial,'  ',lname) as FullName ,count(pno) from works_on inner join employee on essn=ssn group by essn order by count(pno) desc ;
...
select concat(fname,' ',minit,' ',lname) as namee
from employee
...
from works_on 
group by pno 
having numemps = (select min(nemps) from (select count(*) nemps from works_on group by pno) tempproj);


-- 31. Display the fname of those employees who work for at least 20 hours
select fname,hours as HRS from employee inner join works_on on ssn=essn where hours>= 20 group by fname order by hours;
-- 32. What is the average salary of those employees who have at least one
--     dependent
select fname,avg(salary),count(dependent_name)as nmofdependednts from employee inner join dependent on employee.ssn=dependent.essn group by essn having count(dependent_name)>=1 ;

select avg(salary),count(dependent_name) from employee inner join dependent on employee.ssn= dependent.essn group by essn having count(dependent_name)>=1;
...
select ssn,e.fname, ed.dname 'employee-department', p.pname, pd.dname
...
--     average salary?
select dname,avg(salary) from department d inner join employee e on d.dnumber=e.dno group by dno having avg(salary)=(select min(sal) from (select avg(salary) as sal from employee group by dno) tokkoloalias);
select dname,avg(salary) from department d inner join employee e on d.dnumber=e.dno group by dno;

-- 38. Display the fname of the employee along with the fname of the manager

select e.fname 'EmpName', m.fname 'MgrName' from employee e, employee m where e.super_ssn = m.ssn;
...
-- select e.ssn, p.pname, d.dname 'emp-dept-name' ,p.dnum from employee e inner join department d on dno=dnumber inner join project p on d.dnumber=p.dnum having (select d.dname from department inner join project on dnumber=dnum) !=(select d.dname from employee inner join department on dno=dnumber) ;
...
select e.ssn, e.fname, d.dependent_name from employee e left join dependent d on e.ssn = d.essn;
